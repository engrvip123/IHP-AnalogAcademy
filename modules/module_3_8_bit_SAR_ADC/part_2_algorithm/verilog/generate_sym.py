...
def parse_verilog(verilog_file):
    """Extract module name, inputs, and outputs from a Verilog file while formatting bus signals properly."""
    ...
    # Adjusted regex to handle 'input' and 'output' properly while ignoring wire/reg types
    ports = re.findall(r"(input|output)\s*(?:wire|reg)?\s*(\[\d+:\d+\])?\s*(\w+)", code)

    inputs = []
    outputs = []
    for direction, bus, name in ports:
        formatted_name = f"{name}{bus}" if bus else name
        if direction == "input":
            inputs.append(formatted_name)
        else:
            outputs.append(formatted_name)
    
    return module_name, inputs, outputs

def generate_xschem_symbol(verilog_file, so_file, output_sym):
    """Generate an Xschem symbol (.sym) for the given Verilog module."""
    ...
    symbol = [
        ...
        f'vhdl_ignore=true',
        f'format="@name [ {" ".join(f"@@{p}" for p in inputs)} ] [ {" ".join(f"@@{p}" for p in outputs)} ] null @dut .model @dut @d_cosim_model simulation=@model"',
        f'template="name={module_name}',
        f'dut={module_name}',
        f'd_cosim_model={module_name}_cosim',
        f'model={so_file}"}}',
        f'V {{}}', f'S {{}}', f'E {{}}'
    ]
...
def main():
    ...
    if len(sys.argv) != 4:
        print("\nUsage: python generate_sym.py <verilog_file> <so_file> <symbol_file>")
        print("Example: python generate_sym.py control.v test.so test.sym\n")
        sys.exit(1)

    verilog_file, so_file, sym_file = sys.argv[1:4]
    try:
        module_name, inputs, outputs = parse_verilog(verilog_file)
        print(f"Parsed module: {module_name}\nInputs: {inputs}\nOutputs: {outputs}")
        generate_xschem_symbol(verilog_file, so_file, sym_file)
